...
/**
 * Chair Summary OpenAI Service
 * 
 * Handles OpenAI API calls for PO analysis ONLY (not summary generation).
 * Summary generation is handled by NLPSummarizationService.
 * Extracted from ChairSummaryAdapter to centralize OpenAI logic.
 */
class ChairSummaryService
{
    protected $openAIService;
    protected $promptBuilder;

    public function __construct(OpenAIService $openAIService, ChairSummaryPromptBuilder $promptBuilder)
    {
    ...
    /**
     * Generate PO analysis using OpenAI (NO summary generation - summary comes from NLP)
     *
     * @param string $text
     * @param int|null $week
    ...
    public function generateSummaryWithPOAnalysis(
        ...
        try {
            ...
            if ($response['success'] && $response['content']) {
                ...
                // Extract PO analysis from response (NO summary - summary comes from NLP)
                ...
                return [
                    'summary' => '', // Summary is generated by NLP, not OpenAI
                    'usedGPT' => true,
                ...
                Log::error('OpenAI API request failed in ChairSummaryService', [
                    'error' => $response['error'] ?? 'Unknown error'
                ]);
                return $this->getUnavailableResponse();
            }
        } catch (\Throwable $e) {
            Log::error('OpenAI API Error in ChairSummaryService', [
    ...
    /**
     * Normalize summary from OpenAI response
     * NOTE: This method is deprecated - summaries are now generated by NLP, not OpenAI
     * Kept for backward compatibility but should not be used
     */
    private function normalizeSummary(string $content): string
    {
        // Summaries are now generated by NLP, not OpenAI
