...
use App\Services\Coordinator\CoordinatorPOPromptBuilder;
use Illuminate\Support\Facades\Log;

/**
 * Coordinator Summary OpenAI Service
 * 
 * Handles OpenAI API calls for PO analysis ONLY (not summary generation) for coordinator views.
 * Summary generation is handled by OpenAI summarization services.
 */
class CoordinatorSummaryService
{
    protected $openAIService;
    protected $promptBuilder;

    public function __construct(OpenAIService $openAIService, CoordinatorPOPromptBuilder $promptBuilder)
    {
    ...
    /**
     * Generate PO analysis using OpenAI for coordinator (single student)
    ...
    public function generateSummaryWithPOAnalysis(
        ...
        try {
            ...
            // Call OpenAI API with increased timeout for better responses
            ...
            if ($response['success'] && $response['content']) {
                ...
                Log::error('OpenAI API request failed in CoordinatorSummaryService', [
                    'error' => $response['error'] ?? 'Unknown error'
                ]);
                return $this->getUnavailableResponse();
            }
        } catch (\Throwable $e) {
            Log::error('OpenAI API Error in CoordinatorSummaryService', [
